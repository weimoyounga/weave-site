...
export async function onRequest(context) {
    ...
    // ── KV helpers ────────────────────────────────────────────────────────────
    const kv = env.SWATCH_KV;
    if (!kv) {
        return new Response(
            JSON.stringify({ error: 'KV binding SWATCH_KV is missing. Check Pages → Settings → Functions → KV namespace bindings.' }),
            { status: 500, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
        );
    }

    async function getData() {
        const raw = await kv.get('swatch_data');
        ...
        await kv.put('swatch_data', JSON.stringify(initial));
        return initial;
    }

    async function putData(data) {
        data.lastModified = Date.now();
        await kv.put('swatch_data', JSON.stringify(data));
    }
