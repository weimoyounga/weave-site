(function () {
    ...
    const STORAGE_KEY       = 'weave-saved-swatches';
    const GROUPS_STORAGE_KEY = 'weave-swatch-groups';
    ...
    function loadSwatches() {
        try {
            ...
            if (!raw) return DEFAULT_SWATCHES.map(function(s) { return Object.assign({}, s); });
    ...
    function loadGroups() {
        try {
            ...
            if (!raw) return DEFAULT_GROUPS.map(function(g) { return Object.assign({}, g); });
            const parsed = JSON.parse(raw);
            return Array.isArray(parsed) ? parsed : [];
        } catch {
            return [];
    ...
    function confirmEditSwatchMetaFromModal() {
        ...
        const found = all.find(function (x) { return x.id === editSwatchMetaTargetId; });
        if (!found) {
    ...
    function renderSavedList() {
        ...
        list.forEach(function (item) {
            ...
            delBtn.addEventListener('click', function () {
                const next = loadSwatches().filter(function (x) { return x.id !== item.id; });
                saveSwatches(next);
                renderSavedList();
            });
